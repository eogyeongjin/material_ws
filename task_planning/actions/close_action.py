...
class Close(BaseAction):
    ...
    def check_precondition(self, **kwargs) -> tuple[bool, str]:
        ...
        if not target_object['openable']:
            return False, f"{self.command} failed: {target_object['objectType']} is not a closable object"

        if not target_object['isOpen']:
            return False, f"{self.command} failed: {target_object['objectType']} is already closed"
    ...
    def check_success(self, **kwargs) -> tuple[bool, str]:
        ...
        if target_object['isOpen']:
            return False, f"{self.command} failed: {target_object['objectType']} is still open ({self.controller.last_event.metadata['errorMessage']})"
